...
export default function MasterAdminConsole() {
   ...
   const [data, setData] = useState({
      ...
      announcements: [
         { id: 1, title: 'Launch Of New Semester', author: 'Admin User', date: '2026-04-27T10:24' },
         { id: 2, title: 'General Notification', author: 'Admin User', date: '2026-04-27T08:27' },
         { id: 3, title: 'Webinar - AI is Future', author: 'Admin User', date: '2026-04-22T20:00' }
      ],
      events: [
   ...
   const [form, setForm] = useState({
      ...
      profileimageurl: '', roleid: '', cohortIds: []
   });
   ...
   const formatCohortDate = (cohort) => {
      const ts = cohort?.timecreated ?? cohort?.enrollmentDate ?? cohort?.timemodified;
   ...
   const openAddUserModal = () => {
      ...
      if (!data.roles.length) {
         fetch('http://localhost:4000/api/roles').then(r => r.json()).then(res => {
                  ...
                  <div key={key}>
                     ...
                     {mainTab === key && (
                        <div className="ml-8 mt-1 space-y-0.5 py-1 border-l-2 border-primary/20 animate-in slide-in-from-left-1 duration-300">
                           {item.subs.map(sub => (<button key={sub} onClick={() => { setSubTab(sub); if (sub === 'Add user') openAddUserModal(); }} className={`w-full text-left px-5 py-2 text-[10px] font-bold tracking-tight transition-all ${subTab === sub ? 'text-primary' : 'text-muted hover:text-main'}`}>{sub}</button>))}
                        </div>
                     )}
                  </div>
               ))}
                              ...
                              {showModal === 'Add User' && (
                                 <div className="space-y-3 pt-2 border-t border-glass-border">
                                    ...
                                    {(data.cohorts || []).length > 0 ? (
                                       ...
                                       <p className="text-[10px] font-bold text-muted italic">No cohorts available. Create cohorts under Cohort Groups first.</p>
                                    )}
                                 </div>
                              )}
                           </div>
